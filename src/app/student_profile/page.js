...
import { faUser, faKey, faTachometerAlt, faSignOutAlt } from '@fortawesome/free-solid-svg-icons';

export default function StudentProfile() {
  const router = useRouter();
  const { isAuthenticated, isLoading: authLoading, error: authError, user, logout } = useAuth('student');
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/student/profile?userId=${user.userId}`);
        if (!response.ok) {
        ...
        if (data.profile && data.profile.profilePicture) {
          setAvatarSrc(data.profile.profilePicture);
        } else {
  ...
  if (!studentData) return <div className="text-center text-gray-600 py-4">No profile data found</div>;
    ...
    <div className="font-sans bg-gray-100 min-h-screen flex flex-col">
      ...
      <header className="bg-[#4154f1] text-white p-4 shadow-md flex justify-between items-center">
        <div className="flex items-center">
          ...
          />
          <Link href="/student_dashboard" className="text-xl font-bold text-white no-underline">
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <small className="text-gray-200 text-xs">{studentData.userId}</small>
          </div>
        </div>
      </header>
            ...
            <div className="flex flex-col md:flex-row justify-between">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Year: {studentData.year || 'Not specified'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="border-b border-gray-200 pb-4">
                ...
                <div className="text-lg text-gray-900 font-medium">{studentData.userId}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Name</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.name}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Email</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.email}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Phone</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.phone}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Department</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.department}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Program</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.program}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Year</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.year || 'Not specified'}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Semester</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.semester}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Address</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.address}</div>
              </div>
              <div className="border-b border-gray-200 pb-4">
                <div className="text-sm font-medium text-gray-600">Emergency Contact</div>
                <div className="text-lg text-gray-900 font-medium">{studentData.emergencyContact}</div>
              </div>
            </div>
            <div className="mt-6">
              <button
